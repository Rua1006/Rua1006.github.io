...
<head>
    ...
    .title {font-weight: 600; padding-left: 2rem; margin-bottom: 20px;  width: 58%; margin-left: 22%; }
    ...
    .btn-group {width: 25%; float: right; padding-left: calc(var(--bs-gutter-x) * .5); padding-right: 1.5rem!important; margin-right: 14%}
    ...
    <div class="product" id="content">
		...
		<h2 class="title">답변 수정</h2>
			<form name="frm1" id="frm1" action="<%=request.getContextPath() %>/UpdateQnaProCtrl.do" method="post">
				...
				<div class="btn-group">
					<input type="submit" value="글 변경" class="btn btn-dark">
					<a href="<%=request.getContextPath() %>/GetQnaListCtrl.do" class="btn btn-secondary">목록으로</a>
					<a href="<%=request.getContextPath() %>/DeleteQnaCtrl.do?no=<%=vo.getNo() %>" class="btn btn-dark">글 삭제</a>
				</div>
			</form>
		</main>
	</div>
